...
var SECURE_DRIVER_PIN = "9955";       // Driver App PIN
var SECURE_ADMIN_PW = "AdminSecret8822"; // Admin Dashboard Password
...
function doGet(e) {
  try {
    ...
    // Inside doGet(e)...
    if (a == "getCoupons") return getCoupons(); // <-- ADD THIS
    
    // Protected Actions (Requires Password)
...
function doPost(e) {
  try {
    ...
    // Inside doPost...
    if (a == "checkCoupon") return checkCoupon(d.code, d.total); // <-- Pass total
...
function getAdminDashboard(pw) {
  ...
  // Loop through orders (Skipping header)
  for (var i = oData.length - 1; i >= 1; i--) {
    var row = oData[i];
    var oDate = new Date(row[8]); // Date column
    var oTotal = Number(row[5]);
    var oItems = row[7];
    var oUser = row[2]; // Name column
    
    // Add to recent orders list (Limit 50)
    if (orders.length < 50) {
      orders.push({ 
        id: row[0], userId: row[1], name: row[2], phone: row[3], address: row[4], 
...
function placeOrderSecure(d) {
  ...
  try {
    ...
    var c = SS.getSheetByName("Carts"); // <--- ADDED THIS LINE
...
function toggleStore(d) { 
  // d.password is checked in doPost() before reaching here
  
  var s = SS.getSheetByName("Config");
  
  if (!s) {
    return responseJSON({status: "error", message: "Config Sheet not found"});
  }
  ...
  // Search for the Manual_Close key
  for(var i = 1; i < data.length; i++) {
    if(String(data[i][0]) === "Manual_Close") {
  ...
  // Update or Append
  if(found) {
    s.getRange(rowIndex, 2).setValue(d.state);
  } else {
    s.appendRow(["Manual_Close", d.state]);
  }
  
  // Optional: Clear cache if you cache config elsewhere
...
function updateOrderStatus(d) { 
  // No password check here, checked in doPost
...
function getUserData(uid, token){ 
  ...
  for(var i=1;i<ur.length;i++) 
    ...
    if(ur[i][7])p.lang=ur[i][7]; p.points = (ur[i].length > 9) ? Number(ur[i][9]||0) : 0; // --- ADD THIS ---
  ...
  // --- NEW: Filter by Last Month ---
  var recentIds = [];
  if (v) {
    var vData = v.getDataRange().getValues();
    var now = new Date();
    var oneMonthAgo = new Date();
    oneMonthAgo.setMonth(now.getMonth() - 1); // Calculate date 30 days ago

    // Loop backwards (Newest first)
    for (var x = vData.length - 1; x >= 1; x--) {
      // Row structure: [0]Timestamp, [1]UserID, [2]ProductID
      var rowDate = new Date(vData[x][0]);
      
      // Check 1: Is it this user?
      // Check 2: Is the date newer than 1 month ago?
...
function checkStoreStatus() { 
  ...
  // Default to TRUE (Always Open), ignoring the time
  var isOpen = true; 
  
  // Only close if the Manual Switch is ON
  if (c.Manual_Close === true || c.Manual_Close === "true") {
      isOpen = false;
  }
  ...
  return responseJSON({
      status: "success",
      isOpen: isOpen,
      hours: "24/7", // You can hardcode this or keep c.Open_Hour if you prefer
      config: c
  }); 
}

function checkCoupon(code, cartTotal) { // <-- Added cartTotal param
  var s = SS.getSheetByName("Coupons");
  var d = s.getDataRange().getValues();
  
  for(var i=1; i<d.length; i++) {
    // Match Code & Active
    if(String(d[i][0]).toUpperCase() === String(code).toUpperCase() && d[i][2] === true) {
       var min = Number(d[i][3] || 0);
       // Validate Amount
       if (Number(cartTotal) < min) {
         return responseJSON({status: "error", message: "Min order " + min + " DH required"});
       }
       return responseJSON({status:"success", discount:d[i][1], code:d[i][0], min: min});
    }
  }
...
function logView(d) {
  ...
  if (!s) {
    s = SS.insertSheet("Viewed");
    s.appendRow(["Timestamp", "UserID", "ProductID"]);
  }
  // Append: Date, UserID, ProductID
...
function getCoupons() {
  ...
  for (var i = 1; i < data.length; i++) {
    // Check Is_Active (Col C / Index 2)
    if (data[i][2] === true) {
      coupons.push({
        code: data[i][0],
        discount: Number(data[i][1]),
        min: Number(data[i][3] || 0), // Min Amount
        desc: data[i][4] || ""        // Description
      });
    }
  }
