...
function doGet(e) {
  ...
  if (a == "getReviews") return getReviews(e.parameter.productId); // NEW
...
function doPost(e) {
  try {
    ...
    if (a == "submitReview") return submitReview(d); // NEW
...
function getReviews(pid) {
  ...
  for(var i=1; i<data.length; i++) {
    // Col 4 = ProductID (Index 4), Col 7 = Status (Index 7)
    if(String(data[i][4]) === String(pid) && data[i][7] !== 'Hidden') {
      reviews.push({
        user: data[i][3], // UserName
        date: new Date(data[i][1]).toLocaleDateString(),
        rating: data[i][5],
        comment: data[i][6]
      });
...
function submitReview(d) {
  // d = { userId, productId, rating, comment, userName, authToken }
  // Optional: Verify if user bought item? (Skipped for speed, but suggested)
  var s = SS.getSheetByName("Reviews");
  ...
  // Clear catalog cache so stars update if we implement avg in catalog later
...
// --- VALIDATION HELPERS ---
function isValidMoroccanPhone(p) {
  var clean = String(p).replace(/[\s\-\(\)]/g, '');
  var re = /^(?:(?:\+|00)212|0)[67]\d{8}$/; 
...
function updateOrderStatus(d) {
  if (d.adminSecret !== ADMIN_SECRET) return responseJSON({status: "error", message: "Unauthorized"});
  ...
  for (var i = 1; i < r.length; i++) {
    if (String(r[i][0]) === String(d.orderId)) {
      ...
      return responseJSON({status: "success", orderId: d.orderId, newStatus: d.newStatus});
    }
  }
...
function placeOrderSecure(d) {
  if (!isValidMoroccanPhone(d.phone)) return responseJSON({status: "error", message: "Invalid Phone. Use 06... or 07..."});

  if (d.userId && !checkRateLimit(d.userId)) return responseJSON({status: "error", message: "Please wait 30s before placing another order."});

  if (d.userId && d.userId !== "Guest") {
    if (!verifySession(d.userId, d.authToken)) return responseJSON({status: "error", message: "Auth Failed. Log in again."});
  }

  var lock = LockService.getScriptLock();
  try { lock.waitLock(10000); } catch (e) { return responseJSON({status: "error", message: "System busy. Please try again."}); }

  try {
    var s = SS.getSheetByName("Catalog");
    var o = SS.getSheetByName("Orders");
    var u = SS.getSheetByName("Users"); // Access Users for Points
    
    var pData = s.getDataRange().getValues();
    var headers = pData[0];
    
    var idxId = headers.indexOf("ID");
    var idxPrice = headers.indexOf("Price_MAD");
    ...
    for (var k = 0; k < cart.length; k++) {
      ...
      for (var r = 1; r < pData.length; r++) {
        if (String(pData[r][idxId]) === String(item.id)) {
          ...
          if (currentStock < item.qty) return responseJSON({status: "error", message: "Item " + item.title + " is out of stock!"});
          
          s.getRange(rowIndex, idxStock + 1).setValue(currentStock - item.qty);
          sub += (price * item.qty * (Number(item.variantMult)||1));
          il.push(item.qty + "x " + item.title);
          break;
        }
      }
    }
...
function registerUser(d) {
  if (!isValidMoroccanPhone(d.phone)) return responseJSON({status: "error", message: "Invalid Phone. Must be 06... or 07..."});
  if (d.email && !isValidEmail(d.email)) return responseJSON({status: "error", message: "Invalid Email Format"});

  var s = SS.getSheetByName("Users");
  var r = s.getDataRange().getValues();
  for (var i = 1; i < r.length; i++) if (String(r[i][2]) == String(d.phone)) return responseJSON({status: "error", message: "Phone already registered"});
  
  var id = Utilities.getUuid();
  var token = Utilities.getUuid();
  // Added 0 for Points column at the end
  s.appendRow([id, d.name, "'" + d.phone, d.password, new Date(), "MAD", (d.email || ""), "en", token, 0]); 
  
  return responseJSON({status: "success", userId: id, name: d.name, authToken: token});
}

function updateUser(d) {
  if (d.phone && !isValidMoroccanPhone(d.phone)) return responseJSON({status: "error", message: "Invalid Phone"});
  if (d.email && !isValidEmail(d.email)) return responseJSON({status: "error", message: "Invalid Email"});

  var s = SS.getSheetByName("Users");
  var r = s.getDataRange().getValues();
  for (var i = 1; i < r.length; i++) {
    if (String(r[i][0]) === String(d.userId)) {
...
function loginUser(d) {
  ...
  for (var i = 1; i < r.length; i++) {
    if (String(r[i][2]) == String(d.phone) && String(r[i][3]) == String(d.password)) {
      ...
      s.getRange(i + 1, 9).setValue(newToken);
      CacheService.getScriptCache().put("auth_" + r[i][0], newToken, 600);
      return responseJSON({status: "success", userId: r[i][0], name: r[i][1], authToken: newToken});
    }
  }
  return responseJSON({status: "error", message: "Invalid Phone or Password"});
}

function verifySession(uid, token) {
  if (!uid || !token) return false;
  var cache = CacheService.getScriptCache();
  var cachedToken = cache.get("auth_" + uid);
  if (cachedToken && cachedToken === token) return true;

  var sheet = SS.getSheetByName("Users");
  var data = sheet.getDataRange().getValues();
  for (var i = 1; i < data.length; i++) {
    if (String(data[i][0]) === String(uid)) {
      var dbToken = String(data[i][8]);
      if (dbToken === token) {
        cache.put("auth_" + uid, token, 600);
        return true;
      }
    }
  }
...
function getCatalog(){
  ...
  var headers = d[0];
  function getIdx(name) { var index = headers.indexOf(name); if (index > -1) return index; return headers.findIndex(function(h) { return String(h).toLowerCase() === String(name).toLowerCase(); }); }

  var idxID = getIdx("ID"), idxCat = getIdx("Category"), idxTitleEn = getIdx("Title_EN"), idxTitleFr = getIdx("Title_FR"), idxTitleAr = getIdx("Title_AR"), idxDescEn = getIdx("Desc_EN"), idxDescFr = getIdx("Desc_FR"), idxDescAr = getIdx("Desc_AR"), idxPrice = getIdx("Price_MAD"), idxOldPrice = getIdx("Old_Price_MAD"), idxStock = getIdx("Stock"), idxImg = getIdx("Image_URL"), idxTags = getIdx("Tags");

  var products = [];
  // Get Review Summary from Cache or Sheet (Simplified: We won't join here for performance, we fetch reviews on demand in frontend)
...
function updateAddress(d){
  ...
  if(idx>-1){ if(d.type==='Home')s.getRange(idx,2).setValue(as); if(d.type==='Office')s.getRange(idx,3).setValue(as) }else{ s.appendRow([tid,(d.type==='Home'?as:''),(d.type==='Office'?as:'')]) }
  return responseJSON({status:"success"})
}
