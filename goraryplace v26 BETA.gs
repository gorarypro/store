...
function doGet(e) {
  try {
    ...
    return responseJSON({status: "error", message: "Backend Error: " + error.toString()});
  }
}
...
function doPost(e) {
  try {
    ...
    if (a == "toggleStore") return toggleStore(d); // NEW
    ...
    return responseJSON({status: "error", message: "JSON Parse Error"});
  }
}
...
function getAdminDashboard(pw) {
  ...
  // Start from end (newest first)
  for (var i = oData.length - 1; i >= 1; i--) {
    if (orders.length >= 50) break;
    var row = oData[i];
    orders.push({
      id: row[0], userId: row[1], name: row[2], phone: row[3], 
      total: row[5], status: row[9], date: row[8], items: row[7]
    });
    
    // Calc Revenue (Only Completed/Delivered)
    if ((row[9] === "Completed" || row[9] === "Delivered") && String(row[8]).indexOf(todayStr) > -1) {
      revenueToday += Number(row[5]);
    }
  }

  // 2. Get Reservations (Pending only)
  var resData = rSheet ? rSheet.getDataRange().getValues() : [];
  var reservations = [];
  for (var j = 1; j < resData.length; j++) {
    if (resData[j][8] === "Pending") {
      reservations.push({ id: resData[j][0], name: resData[j][3], date: resData[j][5], time: resData[j][6], guests: resData[j][7] });
    }
  }
...
function toggleStore(d) {
  if (d.password !== ADMIN_SECRET) return responseJSON({status: "error", message: "Unauthorized"});
  var s = SS.getSheetByName("Config");
  var data = s.getDataRange().getValues();
  // We use a specific key "Manual_Close" to override hours
  var found = false;
  for(var i=1; i<data.length; i++) {
    if(data[i][0] === "Manual_Close") {
      s.getRange(i+1, 2).setValue(d.state); // true or false
      found = true; 
      break;
    }
  }
  if(!found) s.appendRow(["Manual_Close", d.state]);
...
function updateOrderStatus(d) {
  ...
  for (var i = 1; i < r.length; i++) {
    if (String(r[i][0]) === String(d.orderId)) {
      s.getRange(i + 1, 10).setValue(d.newStatus);
...
// --- STANDARD FUNCTIONS (Keep existing ones below) ---
...
function placeOrderSecure(d) { if (!isValidMoroccanPhone(d.phone)) return responseJSON({status: "error", message: "Invalid Phone."}); if (d.userId && !checkRateLimit(d.userId)) return responseJSON({status: "error", message: "Please wait 30s."}); if (d.userId && d.userId !== "Guest" && !verifySession(d.userId, d.authToken)) return responseJSON({status: "error", message: "Auth Failed."}); var lock = LockService.getScriptLock(); try { lock.waitLock(10000); } catch (e) { return responseJSON({status: "error", message: "System busy."}); } try { var s = SS.getSheetByName("Catalog"); var o = SS.getSheetByName("Orders"); var u = SS.getSheetByName("Users"); var pData = s.getDataRange().getValues(); var headers = pData[0]; function getIdx(n){ return headers.findIndex(function(h){return String(h).toLowerCase()===String(n).toLowerCase()}); } var idxId = getIdx("ID"), idxPrice = getIdx("Price_MAD"), idxStock = getIdx("Stock"), idxExtras = getIdx("Extras_JSON"); var sub = 0.0; var il = []; var cart = []; try { cart = JSON.parse(d.cart); } catch(e) { cart = []; } for (var k = 0; k < cart.length; k++) { var item = cart[k]; var rowIndex = -1; for (var r = 1; r < pData.length; r++) { if (String(pData[r][idxId]) === String(item.id)) { rowIndex = r + 1; var currentStock = Number(pData[r][idxStock]); var price = Number(pData[r][idxPrice]); if (currentStock < item.qty) return responseJSON({status: "error", message: "Item " + item.title + " out of stock!"}); var extraCost = 0; var extraNames = []; if (item.extras && Array.isArray(item.extras) && idxExtras > -1) { var availableExtras = []; try { availableExtras = JSON.parse(pData[r][idxExtras] || "[]"); } catch(e) {} item.extras.forEach(function(reqExtra) { var found = availableExtras.find(function(ex) { return ex.name === reqExtra; }); if (found) { extraCost += Number(found.price); extraNames.push(found.name); } }); } s.getRange(rowIndex, idxStock + 1).setValue(currentStock - item.qty); sub += (price + extraCost) * item.qty * (Number(item.variantMult)||1); var titleStr = item.qty + "x " + item.title; if(extraNames.length > 0) titleStr += " (" + extraNames.join(", ") + ")"; il.push(titleStr); break; } } } var cs = SS.getSheetByName("Config"); var cd = cs.getDataRange().getValues(); var cf = {}; for (var j = 1; j < cd.length; j++) cf[cd[j][0]] = cd[j][1]; var dk = "Delivery_" + d.zone; var df = Number(cf[dk] || 0); if (sub >= (cf.Free_Shipping_Threshold || 200)) df = 0; var di = 0; if (d.couponCode) { var cr = JSON.parse(checkCoupon(d.couponCode).getContent()); if (cr.status === "success") di = sub * cr.discount; } var tot = sub + df - di; if (d.userId && d.userId !== "Guest") { var pointsEarned = Math.floor(tot * 0.10); var uData = u.getDataRange().getValues(); for(var ui = 1; ui < uData.length; ui++) { if(String(uData[ui][0]) === String(d.userId)) { var currentPoints = Number(uData[ui][9] || 0); u.getRange(ui+1, 10).setValue(currentPoints + pointsEarned); break; } } } var id = "ORD-" + Math.floor(Math.random() * 1000000); var fd = Utilities.formatDate(new Date(), "Africa/Casablanca", "yyyy-MM-dd HH:mm:ss"); o.appendRow([String(id), String(d.userId || "Guest"), String(d.name || "Unknown"), "'" + String(d.phone || "No Phone"), String(d.address || d.zone || "N/A"), Number(tot.toFixed(2)), String(d.method || "Cash"), JSON.stringify(il), String(fd), "Pending", "MAD", Number(tot.toFixed(2))]); CacheService.getScriptCache().remove("cat_v156"); return responseJSON({status: "success", orderId: id, secureTotal: tot.toFixed(2)}); } catch (err) { return responseJSON({status: "error", message: "Server Error: " + err.message}); } finally { lock.releaseLock(); } }
function bookTable(d) { if (d.userId && d.userId !== "Guest" && !verifySession(d.userId, d.authToken)) return responseJSON({status: "error", message: "Auth Failed"}); if (!isValidMoroccanPhone(d.phone)) return responseJSON({status: "error", message: "Invalid Phone"}); var s = SS.getSheetByName("Reservations"); if(!s) { s = SS.insertSheet("Reservations"); s.appendRow(["ID","Date","UserId","Name","Phone","ResDate","ResTime","Guests","Status"]); } var id = "RES-" + Math.floor(Math.random()*100000); s.appendRow([id, new Date(), d.userId||"Guest", d.name, "'"+d.phone, d.date, d.time, d.guests, "Pending"]); return responseJSON({status: "success", reservationId: id}); }
...
function checkStoreStatus(){ var s=SS.getSheetByName("Config"),d=s.getDataRange().getValues(),c={}; for(var i=1;i<d.length;i++)c[d[i][0]]=d[i][1]; var n=new Date(new Date().toLocaleString("en-US",{timeZone:c.Timezone||"Africa/Casablanca"})),h=n.getHours(); 
// Manual Override check
var isOpen = (h>=c.Open_Hour&&h<c.Close_Hour);
if (c.Manual_Close === true || c.Manual_Close === "true") isOpen = false;
return responseJSON({status:"success",isOpen:isOpen,hours:c.Open_Hour+":00 - "+c.Close_Hour+":00",config:c}) 
}
