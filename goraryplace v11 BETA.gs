...
function getTranslations() {
  ...
  var cached = c.get("trans_v146"); // Updated version
  ...
  c.put("trans_v146", json, 3600);
  return ContentService.createTextOutput(json).setMimeType(ContentService.MimeType.JSON);
}

// --- USER DATA HANDLING (Updated for Column H: Language) ---
function getUserData(uid){
  ...
  // Get User Profile (Cols: A=ID, B=Name, C=Phone, D=Pass, E=Date, F=Currency, G=Email, H=Language)
  var p={name:"",phone:"",email:"",joined:"", lang:"en"}, // Default lang 'en'
  ...
  for(var i=1;i<ur.length;i++) if(String(ur[i][0])==String(uid)){
    ...
    if(ur[i][7])p.lang=ur[i][7]; // Get Language (Col H)
    break;
  }
  ...
  for(var j=1;j<or.length;j++) if(String(or[j][1])==String(uid)){
    var mt=(or[j][11]!==""&&or[j][11]!=null)?or[j][11]:or[j][5];
    ord.push({id:or[j][0],total:mt,date:or[j][8],status:or[j][9],items:or[j][7]})
  }
...
function registerUser(d){
  ...
  // Append: ID, Name, Phone, Password, Date, Currency, Email, Language
...
function updateUser(d){
  ...
  for(var i=1;i<r.length;i++) if(String(r[i][0])===String(d.userId)){
    ...
    if(d.lang)s.getRange(i+1,8).setValue(d.lang); // Update Language (Col H)
...
function placeOrderSecure(d){
  ...
  if(Array.isArray(c)){
    c.forEach(function(x){
      var pr=0;
      for(var i=1;i<p.length;i++) if(String(p[i][0])===String(x.id)){pr=Number(p[i][3]);break}
...
function getCatalog(){
  var c=CacheService.getScriptCache(),h=c.get("cat_v95");
  if(h)return ContentService.createTextOutput(h).setMimeType(ContentService.MimeType.JSON);
  var s=SS.getSheetByName("Catalog"),d=s.getDataRange().getValues();
  d.shift();
  var p=[];
  d.forEach(function(r){
    if(r[0]!=="")p.push({id:r[0],title:r[1],category:r[2],price_MAD:r[3],price_EUR:r[4],price_USD:r[5],stock:r[9],image:r[10],tags:r[11],desc:r[12]})
  });
  var j=JSON.stringify({status:"success",data:p});
  c.put("cat_v95",j,1200);
