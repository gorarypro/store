...
// Inside the loop...
    ...
    orders.push({ 
      ...
      name: row[2], 
      phone: row[3], 
      address: row[4], // <--- ADDED THIS (It was missing)
      total: row[5], 
      status: row[9], 
      date: row[8], 
      items: row[7] 
    ...
    for (var k = 0; k < cart.length; k++) {
      ...
      for (var r = 1; r < pData.length; r++) {
        if (String(pData[r][idxId]) === String(item.id)) {
          ...
          // Save simplified object for history (Title, Qty, Image, Price)
          // We save the localized title sent from front-end if possible, or fallback
          var histItem = {
            id: item.id,
            title: item.title, // Should be string from frontend now
            qty: item.qty,
