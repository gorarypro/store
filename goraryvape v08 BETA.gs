/* ==========================================================
   GORARY VAPE - BACKEND (V15.0: SYNC FIXED)
   ========================================================== */
...
// ⚠️ REPLACE THIS ID WITH YOUR ACTUAL GOOGLE SHEET ID ⚠️
...
function logProductView(d, u, t) {
  ...
  if(s.getLastRow() === 0) s.appendRow(["Timestamp", "User_Token", "ProductID"]); 
...
function syncUserData(d, u) {
  ...
  try {
    ...
    const colData = getIdx(m, ["Cart_JSON", "Product_IDs", "Items"]);
    ...
    for(let i=1; i<data.length; i++) {
      if(String(data[i][colUser]).trim() === String(u.token).trim()) {
...
function loginUser(ph, pw) {
  ...
  for(let i=1; i<d.length; i++) {
    if(String(d[i][idxPh]).replace(/\D/g,"") === cl) {
...
function registerUser(d) {
  ...
  set("ID", id); set("Name", sanitize(d.name)); set("Phone", "'"+clP); set("Password", h);
  set("JoinDate", new Date()); set("AuthToken", t); set("Points", 0); set("Role", "user"); set("LastLogin", new Date());
  s.appendRow(row);
...
function getCatalogJson(refresh) {
  ...
  if(refresh !== "true") { const c = cache.get("catalog_v15_0"); if(c) return responseJSON({status: "success", data: JSON.parse(c), source: "cache"}); }
  try {
    ...
    try { cache.put("catalog_v15_0", JSON.stringify(products), 21600); } catch(e){}
...
function placeOrderSecure(d,u,t) {
  ...
  try {
    ...
    const set = (k,v) => { const i=getIdx(m,[k]); if(i>-1) row[i]=v; };
    set("Order_ID", id); set("User_ID", u ? u.token : (t||"Guest"));
    set("Customer_Name", sanitize(d.name)); set("Phone", "'"+String(d.phone).replace(/\D/g,""));
    set("Address", sanitize(d.address)); set("Total", d.total); set("Items_JSON", d.cart); set("Date", date); set("Status", "Pending"); set("Coupon", d.coupon||""); set("Points_Used", d.pointsUsed||0);
    set("Payment_Method", d.paymentMethod || "COD");
    s.appendRow(row);
    if(u && d.pointsUsed > 0) {
...
function getUserData(t) {
  ...
  const cR = cD.find(r => String(r[0]).trim() === String(u.token).trim());
  if(cR) cart = cR[1];

  // Get Wishlist
  const wD = getSheet("Wishlists").getDataRange().getValues();
  const wR = wD.find(r => String(r[0]).trim() === String(u.token).trim());
  if(wR) wishlist = wR[1];
...
function repairDatabase(pw) {
  ...
  const schema = {
    ...
    "Viewed": ["Timestamp", "User_Token", "ProductID"]
  };
